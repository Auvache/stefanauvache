<template>
  <section class="py-4 bg-dark-blue">
    <div class="container medium-content text-center fade-in">
      <div>
        <p class="white mb-0">Subscribe to my <span class="fw-bold fst-italic">Food for Thought</span> newsletter for tools and lessons from technology, music, books, and life.</p>
        <div>
          <div class="subscribe d-flex flex-column flex-sm-row justify-content-center align-items-center">
            <component is="script" async data-uid="0396ab84d5" src="https://stefan-bradley.ck.page/0396ab84d5/index.js"></component>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>